...
const ReportsPage = () => {
        ...
        const addToTree = (parts: string[], currentNode: Record<string, FileNode>, fullPath: string) => {
            ...
            if (!currentNode[part]) {
                currentNode[part] = {
                    ...
                    // Note: path construction for folder is tricky without retaining hierarchy. 
                    // Easier: Rebuild path from breadcrumbs during navigation.
                    // Actually, let's store just the name and type, and compute path dynamically?
                    // Or store full path.
                    // MinIO keys: "2026/Q1/Proj/img.jpg"
    ...
    const handleDownload = (node: FileNode) => {
        if (node.type === 'file') {
            ...
            // Download Folder Zip
            // Construct prefix from currentPath + node.name
            // We need the ACTUAL prefix. 
            // Since we built the tree from keys, we can reconstruct the prefix.
            const prefix = [...currentPath, node.name].join('/') + '/'; // Ensure trailing slash
