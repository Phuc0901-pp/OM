...
const UserDashboard = () => {
    ...
    return (
        ...
        >
            ...
            {/* Attendance Card */}
            <GlassCard className="relative overflow-hidden shadow-2xl shadow-green-900/5 !border-green-100/50 dark:!border-green-900/30 dark:bg-slate-900/60">
                <div className="absolute top-0 left-0 w-96 h-96 bg-gradient-to-br from-green-400/10 to-emerald-600/10 dark:from-green-400/5 dark:to-emerald-600/5 rounded-full blur-3xl"></div>
                <div className="relative z-10">
                    <div className="flex items-center justify-between mb-8 border-b border-green-100 dark:border-green-900/30 pb-6">
                        <div className="flex items-center gap-4">
                            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center shadow-lg shadow-green-200 dark:shadow-none">
                                <UserCheck className="w-7 h-7 text-white" />
                            </div>
                            <div>
                                <h2 className="text-2xl font-black text-slate-800 dark:text-white">Chấm công hôm nay</h2>
                                <p className="text-sm font-medium text-slate-500 dark:text-slate-400">{new Date().toLocaleDateString('vi-VN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
                            </div>
                        </div>
                        {attendance && attendance.site_status === 1 && (
                            <div className="flex items-center gap-2 px-4 py-2 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-xl font-bold border border-green-200 dark:border-green-800 animate-pulse">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                <h3 className="font-bold text-slate-800 dark:text-white flex items-center gap-2"><LogIn className="w-5 h-5 text-green-600" /> Check-in</h3>
                                {attendance?.status_checkin === 1 && <CheckCircle2 className="w-5 h-5 text-green-600" />}
                            </div>
                            {attendance?.date_checkin ? (
                                <div className="p-6 bg-green-50/50 dark:bg-green-900/20 rounded-2xl border border-green-100 dark:border-green-800 flex items-center justify-between">
                                    <div>
                                        <p className="text-xs font-bold text-green-600 dark:text-green-400 uppercase mb-1">Thời gian check-in</p>
                                        <p className="text-2xl font-black text-green-700 dark:text-green-300 tracking-tight">{new Date(attendance.date_checkin).toLocaleTimeString('vi-VN')}</p>
                                    </div>
                                    <div className="p-3 bg-green-100 dark:bg-green-900/40 rounded-xl"><Clock className="w-6 h-6 text-green-600 dark:text-green-400" /></div>
                                </div>
                            ) : (
                                <PremiumButton onClick={handleCheckIn} disabled={attendanceLoading} className="w-full h-16 text-lg shadow-green-200 dark:shadow-none bg-gradient-to-r from-green-500 to-emerald-600 text-white border-none hover:shadow-lg" icon={<LogIn className="w-5 h-5" />}>
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                <h3 className="font-bold text-slate-800 dark:text-white flex items-center gap-2"><LogOut className="w-5 h-5 text-orange-600" /> Check-out</h3>
                            ...
                            {attendance?.checkout_requested && !attendance?.checkout_approved && !attendance?.checkout_rejected && (
                                <div className="flex items-center gap-2 p-4 bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 rounded-xl font-bold border border-amber-200 dark:border-amber-800">
                                    <div className="w-2 h-2 rounded-full bg-amber-500 animate-pulse"></div>Đang chờ duyệt
                                </div>
                            )}
                            {attendance?.checkout_rejected && (
                                <div className="p-4 bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400">
                            ...
                            {attendance?.date_checkout ? (
                                <div className="p-6 bg-orange-50/50 dark:bg-orange-900/20 rounded-2xl border border-orange-100 dark:border-orange-800 flex items-center justify-between">
                                    <div>
                                        <p className="text-xs font-bold text-orange-600 dark:text-orange-400 uppercase mb-1">Thời gian check-out</p>
                                        <p className="text-2xl font-black text-orange-700 dark:text-orange-300 tracking-tight">{new Date(attendance.date_checkout).toLocaleTimeString('vi-VN')}</p>
                                    </div>
                                    <div className="p-3 bg-orange-100 dark:bg-orange-900/40 rounded-xl"><Clock className="w-6 h-6 text-orange-600 dark:text-orange-400" /></div>
                                </div>
                            ) : attendance?.checkout_approved ? (
                                <PremiumButton onClick={handleCheckOut} disabled={attendanceLoading} className="w-full h-16 text-lg shadow-orange-200 dark:shadow-none bg-gradient-to-r from-orange-500 to-red-600 text-white border-none hover:shadow-lg" icon={<LogOut className="w-5 h-5" />}>
                                    {attendanceLoading ? 'Đang xử lý...' : 'Check-out ngay'}
                                </PremiumButton>
                            ) : attendance?.checkout_requested ? (
                                <button disabled className="w-full h-16 flex items-center justify-center gap-2 bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 rounded-xl font-bold cursor-not-allowed border border-slate-200 dark:border-slate-700">
                                    <Clock className="w-5 h-5" />Đang chờ duyệt...
                                </button>
                            ) : (
                                <PremiumButton onClick={handleRequestCheckout} disabled={attendanceLoading || !attendance?.status_checkin} className="w-full h-16 text-lg shadow-indigo-200 dark:shadow-none bg-gradient-to-r from-indigo-500 to-purple-600 text-white border-none hover:shadow-lg" icon={<LogOut className="w-5 h-5" />}>
