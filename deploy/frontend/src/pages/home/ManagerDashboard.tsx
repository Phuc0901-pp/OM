                        ...
                        {pendingCheckouts.map((req: PendingCheckout) => ( // Explicit type
                            <div key={req.id} className="p-5 bg-white/60 hover:bg-white rounded-2xl border border-orange-100 hover:border-orange-300 shadow-sm hover:shadow-md transition-all flex flex-col justify-between backdrop-blur-sm">
                                <div>
                                    ...
                                    {req.checkout_img_url && (
                                        <div className="mb-4 relative group cursor-pointer overflow-hidden rounded-xl border border-slate-200" onClick={() => setSelectedImage(req.checkout_img_url || null)}>
                                            <img src={req.checkout_img_url} alt="Checkout" className="w-full h-32 object-cover transition-transform group-hover:scale-105" />
