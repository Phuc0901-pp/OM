        ...
        return new Promise((resolve, reject) => {
            ...
            navigator.geolocation.getCurrentPosition(
                async (pos) => {
                    ...
                    // Reverse Geocoding via Nominatim (Free, requires attribution)
                    // Note: In production, consider using a backend proxy or paid service for reliability.
                    try {
                        const response = await fetch(
                            ...
                            {
                                headers: {
                                    'User-Agent': 'RaitekO&M/2.0 (web-client)', // Identify app
                                }
                            }
                        );
                        if (response.ok) {
