...
import {
    ClipboardList, FolderOpen, CheckCircle2,
...
const TaskHierarchy: React.FC<TaskHierarchyProps> = ({
            ...
            <div key={groupKey}>
                <div className="flex items-center gap-2 px-2 pb-2">
                    ...
                    {allDone ? (
                        ...
                        <span className="text-[11px] font-bold text-slate-400 shrink-0 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">
                            {done}/{tasks.length}
                        </span>
                    )}
                </div>
                <div className="bg-white dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-700/60 shadow-sm overflow-hidden">
    ...
    const renderCategoryGroup = (group: { childCatName: string; tasks: FlatTask[] }, i: number) => {
        ...
        return (
            <div key={`cat-${resolvedActive}-${group.childCatName}-${i}`}>
                ...
                <div className="flex items-center gap-2 px-2 pb-2">
                    <FolderOpen className={`w-4 h-4 shrink-0 ${allDone ? 'text-emerald-500' : 'text-indigo-500'}`} strokeWidth={2.5} />
                    <span className="flex-1 text-xs font-bold text-indigo-600 dark:text-indigo-400 truncate">{group.childCatName}</span>
                    {allDone ? (
                        <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0" />
                    ) : (
                        <span className="text-[11px] font-bold text-slate-400 shrink-0 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">
                        ...
                        return (
                            <div key={`${bucket.assetName}-${bi}`}>
                                ...
                                <div className={`flex items-center gap-2 px-4 py-2 ${
                                    bi > 0 ? 'border-t-2 border-slate-100 dark:border-slate-700/60 mt-0' : ''
                                } bg-slate-50/60 dark:bg-slate-800/40`}>
                                    <span className="w-1.5 h-1.5 rounded-full shrink-0 bg-indigo-400" />
                                    <span className="text-[11px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider flex-1">
                                        {bucket.assetName}
                                    </span>
                                    {bucketAllDone ? (
    ...
    return (
        <div className="flex flex-col gap-0 pb-6">
            ...
            <div className="sticky top-0 z-20 bg-slate-50/95 dark:bg-slate-950/95 backdrop-blur-sm border-b border-slate-200 dark:border-slate-700/60 px-3 pt-3 pb-2 space-y-2">
            ...
            {showSuggestions && suggestions.length > 0 && !activeFilter && dropdownRect && (
                ...
                >
                    <div className="px-4 pt-3 pb-1.5 border-b border-slate-100 dark:border-slate-800">
                    ...
                    <div className="py-1 max-h-64 overflow-y-auto">
                        {suggestions.map((s, i) => {
                            ...
                            const chipMap = {
                                mainCat: 'text-amber-700 bg-amber-50 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800',
                                childCat: 'text-emerald-700 bg-emerald-50 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800',
                                asset: 'text-indigo-700 bg-indigo-50 border-indigo-200 dark:bg-indigo-900/30 dark:text-indigo-400 dark:border-indigo-800',
                            };
                            ...
                            return (
                                <button
                                    ...
                                    className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 dark:hover:bg-slate-800/60 text-left transition-colors group"
                                >
                                    <div className={`p-1 rounded-lg ${s.type === 'mainCat' ? 'bg-amber-50 dark:bg-amber-900/30' : s.type === 'childCat' ? 'bg-emerald-50 dark:bg-emerald-900/30' : 'bg-indigo-50 dark:bg-indigo-900/30'}`}>
                                        {iconMap[s.type]}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        {s.breadcrumb && (
                                            <p className="text-[10px] text-slate-400 dark:text-slate-500 font-semibold mb-0.5 truncate">
                                                {s.breadcrumb} ›
                                            </p>
                                        )}
                ...
                {viewMode === 'asset' && activeAssetGroups.map((group, i) =>
                    renderGroup(
                        ...
                        <div className="flex flex-wrap items-center gap-x-1.5 gap-y-0.5 min-w-0">
                            <span className="text-xs font-bold text-slate-800 dark:text-slate-300 uppercase tracking-wider leading-tight">{group.mainCatName}</span>
                            <span className="text-slate-300 dark:text-slate-600 shrink-0">›</span>
                            <span className="text-xs font-bold text-indigo-600 dark:text-indigo-400 leading-tight">{group.childCatName}</span>
                        </div>,
                        ft => makeTaskLabel(ft),
