...
const ProjectHeader: React.FC<ProjectHeaderProps> = ({ selectedAssign, taskDetails }) => {
    ...
    // Logic: Group tasks by SubWork to calculate progress
    const { totalChildCats, completedChildCats, progressPercent } = useMemo(() => {
        const tasksByChildCat: Record<string, DetailAssign[]> = {};
        taskDetails.forEach(task => {
            const childId = task.config?.sub_work?.id || 'uncategorized';
    ...
    if (selectedAssign.end_time) {
        ...
        deadlineElement = (
            <span className="flex items-center gap-1.5 text-xs font-medium text-slate-300 bg-white/5 px-2 py-1 rounded-md">
                <Clock className="w-3 h-3" />
                Cập nhật: {now.toLocaleTimeString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh', hour: '2-digit', minute: '2-digit' })}
            </span>
        );
    }
                            ...
                            <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-slate-300 text-sm font-medium">
                                ...
                                {selectedAssign.start_time && (
                                    <div className="flex items-center gap-2 group/item">
                                        ...
                                        <span>{new Date(selectedAssign.start_time).toLocaleDateString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' })}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                ...
                {selectedAssign.note_assign && (
                    <div className="bg-amber-50/80 backdrop-blur-sm border-t border-amber-100/50 p-4 px-6 flex items-start gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm font-medium text-amber-900 leading-relaxed">{selectedAssign.note_assign}</p>
                        </div>
                    </div>
                )}
            </GlassCard>
