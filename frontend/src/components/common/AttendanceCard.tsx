...
const AttendanceCard: React.FC<AttendanceCardProps> = ({ data, type, onImageClick }) => {
 ...
 return (
 <div className="bg-gradient-to-br from-slate-50 to-indigo-50/30 rounded-xl p-4 space-y-3 border border-slate-100 ">
 ...
 {data.user?.full_name && (
 <span className="flex items-center gap-1 bg-white px-2.5 py-1.5 rounded-lg border border-slate-200 font-medium text-slate-700 ">
 ...
 {/* Checkin */}
 <div className={`rounded-lg p-3 ${isCheckin ? 'ring-2 ring-emerald-400 bg-emerald-50 ' : 'bg-white '} border border-slate-200 `}>
 ...
 {/* Checkout */}
 <div className={`rounded-lg p-3 ${isCheckout ? 'ring-2 ring-indigo-400 bg-indigo-50 ' : 'bg-white '} border border-slate-200 `}>
 ...
 <div className="text-xs bg-red-50 text-red-600 p-2 rounded-lg border border-red-100 ">
...
const MultiImageCell = ({ src, label, onImageClick }: { src?: string, label: string, onImageClick: (src: string, alt: string) => void }) => {
 ...
 alt={`${label} ${index + 1}`}
 className="w-16 h-16 object-cover rounded-lg border border-slate-200 cursor-pointer hover:ring-2 hover:ring-indigo-500 transition-all"
