...
const NotificationDetailModal: React.FC<NotificationDetailModalProps> = ({
    ...
    const contextInfo = React.useMemo(() => {
        ...
        if (taskData.length > 0) {
            ...
            mainCat = t.child_category?.main_category?.name || t.config?.sub_work?.work?.name || '';
            const pAsset = t.config?.asset?.parent?.name;
            const cAsset = t.station?.name || t.station_name || t.config?.asset?.name || '';
            station = pAsset && cAsset ? `${pAsset} - ${cAsset}` : cAsset;
            childCat = t.child_category?.name || t.config?.sub_work?.name || '';
        }
                                                    ...
                                                    <div className="flex-1">
                                                        ...
                                                        <div className="flex items-center gap-4 mt-1 text-xs text-slate-500 dark:text-slate-400">
                                                            <span className="flex items-center gap-1">
                                                                ...
                                                                {format(parseSafeDate(notification.created_at), "HH:mm, dd/MM/yyyy", { locale: vi })}
                                                            </span>
                                                        </div>
