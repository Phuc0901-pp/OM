...
function VirtualTable<T>({
    ...
    if (isLoading) {
        return (
            <div className={`overflow-hidden rounded-xl border border-slate-100 dark:border-slate-700 ${className}`}>
                <table className="w-full text-sm">
                    <thead>
                        <tr className="bg-slate-50 dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700 text-slate-500 text-xs uppercase">
                            {columns.map((col, i) => (
    ...
    if (!data.length) {
        return (
            <div className={`text-center text-slate-400 dark:text-slate-500 py-16 ${className}`}>
    ...
    return (
        <div
            className={`overflow-hidden rounded-xl border border-slate-100 dark:border-slate-700 ${className}`}
        >
            {/* Sticky header */}
                ...
                <thead>
                    <tr className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                        {columns.map((col, i) => (
                            <th
                                key={i}
                                className={`py-3.5 px-5 text-[11px] font-bold tracking-wider uppercase text-slate-400 dark:text-slate-500 ${col.headerClassName ?? 'text-left'}`}
                        ...
                        {virtualItems.map((virtualRow) => {
                            ...
                            return (
                                <tr
                                    ...
                                    className={`border-b border-slate-50 dark:border-slate-800 transition-colors ${onRowClick
            ...
            {/* Row count badge */}
            <div className="px-4 py-2 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 text-xs text-slate-400 dark:text-slate-500 text-right">
