...
const TaskItem: React.FC<{ task: TaskDetail; onImageClick: (src: string, alt: string) => void }> = ({ task, onImageClick }) => {
 ...
 if (isApproved) return { color: 'text-emerald-600 bg-emerald-50 border-emerald-100', icon: <CheckCircle className="w-3.5 h-3.5" />, label: 'Đã duyệt' };
 if (isRejected) return { color: 'text-red-600 bg-red-50 border-red-100', icon: <XCircle className="w-3.5 h-3.5" />, label: 'Từ chối' };
 if (hasSubmitted) return { color: 'text-blue-600 bg-blue-50 border-blue-100', icon: <Clock className="w-3.5 h-3.5" />, label: 'Đang chờ duyệt' };
 return { color: 'text-slate-500 bg-slate-50 border-slate-100', icon: <Clock className="w-3.5 h-3.5" />, label: 'Chưa thực hiện' };
 };
 ...
 {task.updated_at && (
 <span className="flex items-center gap-1 text-xs text-slate-400 px-2 py-1 bg-slate-50 rounded-md border border-slate-100 ">
 <Clock className="w-3.5 h-3.5" />
 {format(parseSafeDate(task.updated_at), "dd/MM/yyyy HH:mm", { locale: vi })}
 </span>
 )}
 {task.assigned_user_names && (
 <span className="flex items-center gap-1 text-xs font-medium text-slate-600 px-2 py-1 bg-slate-50 rounded-md border border-slate-100 ">
 ...
 <div className="mt-3 text-xs text-slate-600 bg-slate-50 p-3 rounded-lg flex gap-2 items-start">
 ...
 <div
 key={idx}
 className="relative shrink-0 w-20 h-20 rounded-lg overflow-hidden border border-slate-200 cursor-pointer group"
