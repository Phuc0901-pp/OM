...
import { Menu, ChevronDown, User, Settings, LogOut, MapPin, Sun, Cloud, CloudRain, CloudLightning, Snowflake, CloudSun, CloudFog, CloudDrizzle } from 'lucide-react';
...
const Header = ({ title, user, onMenuClick }: HeaderProps) => {
 ...
 const { time: currentTime } = useSyncedTime();
 ...
 const dateStr = currentTime.toLocaleDateString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh', day: '2-digit', month: '2-digit', year: '2-digit' });
 const timeStr = currentTime.toLocaleTimeString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh', hour: '2-digit', minute: '2-digit' });
 ...
 return (
 <header className="min-h-[calc(5rem+env(safe-area-inset-top))] h-auto py-2 pt-[calc(env(safe-area-inset-top)+0.5rem)] flex items-center justify-between px-6 md:px-8 fixed top-0 right-0 left-0 md:left-72 z-40 transition-[left] duration-300 bg-white border-b border-slate-200 transition-colors">
 ...
 {/* Desktop Title */}
 <h2 className="hidden md:block text-xl font-black text-slate-800 tracking-tight truncate bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent">
 {title}
 </h2>

 {/* Mobile Title + Clock + Location */}
 <div className="md:hidden flex flex-col min-w-0">
 <h2 className="text-lg font-black text-slate-800 tracking-tight leading-none bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent truncate">
 {mobileTitle}
 </h2>
 <div className="flex flex-col mt-0.5">
 <div className="flex items-center gap-3 mb-1">
 <span className="text-[10px] font-bold text-slate-400 font-mono leading-none">{timeStr}</span>
 {weather && (
 ...
 <span className="text-[10px] font-bold text-slate-600 leading-none">{weather.temp}°</span>
 </div>
 )}
 </div>
 {deviceLocation && (
 <div className="flex items-start gap-1 text-[11px] font-medium text-slate-500 leading-tight">
 ...
 {deviceLocation && (
 <div
 className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 rounded-lg border border-indigo-100 text-indigo-700 text-sm font-bold shadow-sm transition-all hover:shadow-md cursor-help"
 ...
 <span className="text-sm font-bold text-slate-700 ">{weather.temp}°C</span>
 </div>
 )}
 <DigitalClock />
 </div>
 ...
 {(typeof user?.role === 'object' ? user?.role?.name : user?.role) || user?.role_name || 'Guest'}
 </p>
 </div>
