...
interface AddUserModalProps {
    ...
    onClose: () => void;
    onSuccess: () => void;
    userToEdit?: any; // Dữ liệu User cần sửa
}
...
const AddUserModal: React.FC<AddUserModalProps> = ({ isOpen, onClose, onSuccess, userToEdit }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (userToEdit) {
                setFormData({
                    full_name: userToEdit.name || userToEdit.full_name || '',
                    email: userToEdit.email || '',
                    number_phone: userToEdit.number_phone || '',
                    password: '', // Để trống mật khẩu nếu không muốn đổi
                    role_id: userToEdit.role_id || userToEdit.role?.id || '',
                    team_id: userToEdit.team_id || userToEdit.team?.id || ''
                });
            } else {
                setFormData({
                    full_name: '',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(err.response?.data?.error || (userToEdit ? 'Lỗi khi cập nhật' : 'Lỗi khi tạo mới'));
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 animate-fade-in">
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-100">
                    <h2 className="text-xl font-bold text-gray-800">{userToEdit ? 'Chỉnh sửa thông tin' : 'Thêm người dùng mới'}</h2>
                    <button
                        onClick={onClose}
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400 transition-all"
                                placeholder={userToEdit ? "Để trống nếu không đổi" : "Tối thiểu 6 ký tự"}
                            />
                        </div>
                    </div>
