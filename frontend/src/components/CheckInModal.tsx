...
const CheckInModal: React.FC<CheckInModalProps> = ({ isOpen, onClose, onSubmit, loading, mode = 'checkin' }) => {
    const [photos, setPhotos] = useState<CheckInPhotos>({
        personnel_photo: [], id_card_front: [], id_card_back: [], safety_card_front: [], safety_card_back: [], tools_photos: [], documents_photos: []
    });
    ...
    const handleCameraCapture = async (data: string | Blob | (string | Blob)[]) => {
        if (!currentField) {
        ...
        if (base64Items.length > 0) {
            setPhotos(prev => {
                if (currentField.isArray) {
                    return { ...prev, [currentField.field]: [...(prev[currentField.field] as string[]), ...base64Items] };
                } else {
                    return { ...prev, [currentField.field]: base64Items[0] };
                }
            });
        }
    ...
    const handleDeletePhoto = (field: keyof CheckInPhotos, index?: number) => {
        if (Array.isArray(photos[field]) && index !== undefined) {
            setPhotos(prev => ({ ...prev, [field]: (prev[field] as string[]).filter((_, i) => i !== index) }));
        } else {
            // Unlikely to hit unless user passes no index
    ...
    const handleSubmit = () => {
        ...
        if (photos.id_card_front.length > 0) photoData.id_card_front = photos.id_card_front;
        if (photos.id_card_back.length > 0) photoData.id_card_back = photos.id_card_back;
        if (photos.safety_card_front.length > 0) photoData.safety_card_front = photos.safety_card_front;
        if (photos.safety_card_back.length > 0) photoData.safety_card_back = photos.safety_card_back;
                ...
                <div className="fixed inset-0 z-[9999] flex items-end md:items-center justify-center bg-black/60 backdrop-blur-md transition-all">
                    ...
                    <div className="relative w-full h-full md:h-auto md:max-w-3xl md:max-h-[90vh] bg-white md:rounded-3xl shadow-2xl flex flex-col overflow-hidden safe-top safe-bottom">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 bg-slate-50/50">
                            ...
                            {mode === 'checkin' && (
                                <>
                                    <MultiPhotoItem icon={CreditCard} title="CCCD - Mặt trước" required={false} photos={photos.id_card_front} onCapture={() => openCamera('id_card_front', true)} onDelete={(index) => handleDeletePhoto('id_card_front', index)} onPreview={setPreviewImage} color="indigo" />
                                    <MultiPhotoItem icon={CreditCard} title="CCCD - Mặt sau" required={false} photos={photos.id_card_back} onCapture={() => openCamera('id_card_back', true)} onDelete={(index) => handleDeletePhoto('id_card_back', index)} onPreview={setPreviewImage} color="purple" />
                                    <MultiPhotoItem icon={Shield} title="Thẻ an toàn - Mặt trước" required={false} photos={photos.safety_card_front} onCapture={() => openCamera('safety_card_front', true)} onDelete={(index) => handleDeletePhoto('safety_card_front', index)} onPreview={setPreviewImage} color="green" />
                                    <MultiPhotoItem icon={Shield} title="Thẻ an toàn - Mặt sau" required={false} photos={photos.safety_card_back} onCapture={() => openCamera('safety_card_back', true)} onDelete={(index) => handleDeletePhoto('safety_card_back', index)} onPreview={setPreviewImage} color="green" />
