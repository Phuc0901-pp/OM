...
const CheckInModal: React.FC<CheckInModalProps> = ({ isOpen, onClose, onSubmit, loading }) => {
    const [photos, setPhotos] = useState<CheckInPhotos>({
        personnel_photo: null, id_card_front: null, id_card_back: null, safety_card_front: null, safety_card_back: null, tools_photos: [], documents_photos: []
    });
    ...
    const handleCameraCapture = (data: string | Blob) => {
        ...
        if (typeof data === 'string') {
            if (currentField.isArray) {
                setPhotos(prev => ({ ...prev, [currentField.field]: [...(prev[currentField.field] as string[]), data] }));
            } else {
                setPhotos(prev => ({ ...prev, [currentField.field]: data }));
            }
        } else {
    ...
    const handleSubmit = () => {
        if (!photos.personnel_photo) {
            alert('Vui lòng chụp đầy đủ các ảnh bắt buộc!');
            return;
        }

        // Filter out null values before sending
        const photoData: any = {};
        if (photos.personnel_photo) photoData.personnel_photo = photos.personnel_photo;
    ...
    const isComplete = !!photos.personnel_photo;

    if (!isOpen) return null;

    return (
        <>
            <CameraModal isOpen={showCamera} onClose={() => { setShowCamera(false); setCurrentField(null); }} onCapture={handleCameraCapture} />

            <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/60 backdrop-blur-md transition-all">
                ...
                <div className="relative w-full h-full md:h-auto md:max-w-3xl md:max-h-[90vh] bg-white md:rounded-3xl shadow-2xl flex flex-col overflow-hidden safe-top safe-bottom">
                    ...
                    {/* Header */}
                    <div className="flex-none bg-gradient-to-r from-emerald-600 to-teal-600 p-4 md:p-6 shadow-lg z-10">
                        <div className="flex items-center justify-between text-white">
                            <div className="flex items-center gap-3">
                                <div className="p-2.5 bg-white/20 backdrop-blur-md rounded-xl"><Camera className="w-6 h-6" /></div>
                                <div>
                                    <h2 className="text-xl md:text-2xl font-black tracking-tight">Check-in Checklist</h2>
                                    <p className="text-emerald-100 font-medium text-xs md:text-sm">Chụp ảnh xác thực tại dự án</p>
                                </div>
                            </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 bg-slate-50/50">
                        <SinglePhotoItem icon={User} title="Ảnh nhân sự" required photo={photos.personnel_photo} onCapture={() => openCamera('personnel_photo')} onDelete={() => handleDeletePhoto('personnel_photo')} onPreview={setPreviewImage} color="blue" />
        ...
        <div className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all">
            <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                    ...
                    {photo ? (
                        <div className="relative group overflow-hidden rounded-xl border-2 border-emerald-500/50">
                            ...
                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center gap-3 backdrop-blur-[2px]">
                                <button onClick={() => onPreview(photo)} className="p-2 bg-white/20 hover:bg-white/40 rounded-xl text-white backdrop-blur-md transition-all"><ZoomIn className="w-5 h-5" /></button>
                                <button onClick={onDelete} className="p-2 bg-red-500/80 hover:bg-red-600 rounded-xl text-white backdrop-blur-md transition-all shadow-lg shadow-red-500/30"><Trash2 className="w-5 h-5" /></button>
                            </div>
                    ...
                    {photos.length > 0 && (
                        <div className="grid grid-cols-3 gap-3 mb-3">
                            {photos.map((photo, index) => (
                                <div key={index} className="relative group rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all border border-slate-200">
                                    <img src={photo} alt={`${title} ${index + 1}`} className="w-full h-24 object-cover" />
                                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center gap-2">
                                        <button onClick={() => onPreview(photo)} className="p-1.5 bg-white/20 hover:bg-white/40 rounded-lg text-white"><ZoomIn className="w-4 h-4" /></button>
                                        <button onClick={() => onDelete(index)} className="p-1.5 bg-red-500/80 hover:bg-red-600 rounded-lg text-white"><Trash2 className="w-4 h-4" /></button>
                                    </div>
                                </div>
                            ))}
                        </div>
