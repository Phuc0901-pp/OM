...
export function useOfflineStorage(): UseOfflineStorageResult {
 ...
 await updateCounts();
 // NOTE: We intentionally do NOT trigger syncQueue.processQueue() here.
 // Manual submit in EnvironmentPage handles online upload directly.
 // Auto-sync (every 30s) handles offline recovery only.
 return id;
 } catch (error) {
