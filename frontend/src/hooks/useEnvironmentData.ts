...
export const useEnvironmentData = (): UseEnvironmentDataReturn => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            // Filter locally by User ID and Status (Chỉ lấy dự án chưa hoàn thành)
            const assignsData = allAssigns.filter(a => {
                if (a.status_assign === true) return false; // Chỉ lấy status_assign false / null
                if (!a.id_user) return false;
                try {
                    let usersArr: string[] = [];
                    if (Array.isArray(a.id_user)) usersArr = a.id_user;
                    else if (typeof a.id_user === 'string') usersArr = JSON.parse(a.id_user);
                    return usersArr.includes(userId);
                } catch { return false; }
            });
