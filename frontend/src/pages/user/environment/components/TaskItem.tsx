...
const MicroCard: React.FC<{
    ...
    onEdit: () => void;
    usersMap: Record<string, string>;
}> = ({ task, processName, draftCaptures, draftNote, onExpand, onEdit, usersMap }) => {
    ...
    return (
        <button
            onClick={onExpand}
            className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group"
        >
            {/* Status dot */}
            ...
            <div className="flex items-center gap-2 shrink-0">
                {imageCount > 0 && (
                    <span className="flex items-center gap-1 text-[11px] font-semibold text-slate-500 bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded-full">
...
const TaskItem: React.FC<TaskItemProps> = React.memo(({
            ...
            <div className="border-b border-slate-100 dark:border-slate-700/50 last:border-b-0">
                ...
                <AnimatePresence>
                    {microExpanded && (
                        ...
                        >
                            <div className="px-4 pb-4 pt-2 bg-slate-50/60 dark:bg-slate-800/30 border-t border-slate-100 dark:border-slate-700/50">
                                {/* Status Banner */}
                                ...
                                {draftNote && draftNote.trim() && (
                                    <p className="mt-2 text-xs text-slate-600 dark:text-slate-400 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg px-3 py-2 line-clamp-3">
    ...
    return (
        <div className="p-4 flex flex-col gap-3 border-b border-slate-100 dark:border-slate-700/50 last:border-b-0">
            {/* Header row */}
            <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                    <CameraIcon className="w-4 h-4 text-slate-400 shrink-0" />
                    <span className="text-sm font-semibold text-slate-800 dark:text-white truncate">{processName}</span>
                </div>
                    ...
                    >
                        <div className="flex gap-2">
                            <textarea
                                ...
                                className="flex-1 px-3 py-2 text-sm border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 resize-none outline-none text-slate-800 dark:text-white"
                                rows={2}
                                autoFocus
                            />
                            <button
            ...
            {/* Action row */}
            <div className="flex flex-nowrap items-center justify-between gap-y-3 gap-x-2 pt-2 pb-1 border-t border-slate-100 dark:border-slate-700/40 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
