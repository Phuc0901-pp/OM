...
import { Camera as CameraIcon, Save, X, RotateCcw, AlertOctagon as OctagonAlertIcon, CheckCircle2, ScrollText } from 'lucide-react';
                        ...
                        <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
                            {draftCaptures.map((item, idx) => {
                                ...
                                return (
                                    ...
                                    >
                                        {/* Image/Video */}
                                        {typeof item === 'string' && !item.startsWith('blob:') && item.includes('.webm') ? (
                                            <video src={url} className="w-full h-full object-cover" />
                                        ) : (
                                            <img src={url} alt="draft" className="w-full h-full object-cover" onError={(e) => {
                                                // If image fails, try video placeholder? 
                                                // Simpler: assume image for now as per original code
                                                (e.target as HTMLImageElement).src = 'placeholder_error.png';
                                            }} />
                                        )}

                                        {/* Sync Indicator for Local Items */}
                                        {(item instanceof Blob || (typeof item === 'string' && item.startsWith('blob:'))) && (
                                        ...
                                        {isEditing && (
                                            <button
                                                onClick={(e) => { e.stopPropagation(); onDeleteImage(item, idx); }}
                                                className="absolute inset-0 bg-black/40 flex items-center justify-center text-white opacity-100 hover:bg-black/60 transition-colors"
                                            >
                                                <X className="w-6 h-6" />
                                            </button>
                                        )}
