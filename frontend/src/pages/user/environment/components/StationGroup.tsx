...
import { DetailAssign, Config } from '../types';
...
const StationGroup: React.FC<StationGroupProps> = React.memo(({
    ...
    const sortedTasks = React.useMemo(() => {
        return [...tasks].sort((a, b) => {
            const nameA = (a.process?.name || '').toLowerCase();
            const nameB = (b.process?.name || '').toLowerCase();
                        ...
                        {tasks.map(t => {
                            ...
                            return (
                                <div
                                    ...
                                    title={t.process?.name || 'Quy trình'}
                                />
                            );
                        })}
                    </div>
                ...
                <div className="pl-4 pb-2 space-y-2">
                    {sortedTasks.map(task => (
                        <TaskItem
                            key={task.id}
                            task={task}
                            processName={task.process?.name || 'Quy trình'}
                            config={config}
                            isEditing={editingTasks.has(task.id)}
                            isSubmitting={submittingTasks.has(task.id)}
                            draftCaptures={draftCaptures[task.id] || []}
                            draftNote={draftNotes[task.id]}
                            onEdit={() => onTaskEdit(task.id)}
                            onSubmit={(note) => onTaskSubmit(task.id, note)}
