...
const SettingsPageContent = () => {
 ...
 <span className="ml-4 text-gray-600 font-medium ">{t('common.loading')}</span>
 </div>
 );
 }
 ...
 return (
 <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-4 md:p-6 transition-colors duration-500">
 ...
 <p className="text-gray-600 font-medium">{t('settings.subtitle')}</p>
 </div>
 </div>
 ...
 {user.role?.name || user.role_model?.name || 'User'}
 </span>
 <span className="text-sm text-gray-500 bg-gray-100 px-3 py-2 rounded-xl font-medium">
 {t('settings.joined')} {new Date(user.created_at).toLocaleDateString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' })}
 </span>
 </div>
 </div>
 </div>
 ...
 <p className="font-bold text-gray-900 ">{user.role?.name || user.role_model?.name || 'User'}</p>
 </div>
 </div>
 ...
 className="px-4 py-2 border-2 border-gray-200 rounded-xl text-sm font-bold outline-none bg-white transition-all cursor-pointer"
 >
