...
import { List, Calendar, User, Search, Filter, ArrowUpDown, ArrowUp, ArrowDown, Settings2, X, CalendarRange, PlayCircle } from 'lucide-react';
...
interface TaskTableProps {
 ...
 // Controlled from parent (OverviewCards sync)
 statusFilter?: string;
...
const TaskTable: React.FC<TaskTableProps> = ({ assigns, loading, usersMap = {}, statusFilter: externalStatusFilter, onStatusFilterChange }) => {
 const navigate = useNavigate();
 const [searchTerm, setSearchTerm] = useState('');
 // Use external filter from OverviewCards if provided, else internal state
 ...
 <h3 className="text-base font-bold text-slate-800 ">Chi tiết công việc</h3>
 {rows.length > 0 && (
 <span className="ml-auto text-xs bg-indigo-50 text-indigo-700 px-2.5 py-1 rounded-full font-bold border border-indigo-100 ">
 ...
 <div className="flex flex-col lg:flex-row lg:items-center gap-3 px-6 py-4 border-b border-slate-100 bg-slate-50/50 shrink-0">
 ...
 className="w-full pl-9 pr-4 py-2 text-sm bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500/50 shadow-sm transition-all"
 />
 </div>
 ...
 className="w-full pl-9 pr-9 py-2 text-sm font-medium text-center sm:text-left text-slate-700 bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500/50 shadow-sm appearance-none cursor-pointer transition-all"
 ...
 {showAdvancedFilters && (
 <div className="px-6 py-5 border-b border-indigo-100 bg-indigo-50/30 shrink-0 select-none">
 ...
 {/* Table Container */}
 <div className="flex-1 overflow-auto bg-white relative scrollbar-thin scrollbar-thumb-slate-200 ">
 <table className="w-full text-sm text-left min-w-[900px]">
 <thead className="text-xs text-slate-400 uppercase bg-slate-50 sticky top-0 z-10 shadow-sm pointer-events-none">
 <tr>
 <th scope="col" className="w-[18%]">
 <button type="button" onClick={() => handleSort('projectName')} className="flex items-center gap-1.5 px-5 py-4 font-bold tracking-wider hover:text-slate-700 w-full group pointer-events-auto">
 Dự án {renderSortIcon('projectName')}
 </button>
 </th>
 <th scope="col" className="w-[21%]">
 <button type="button" onClick={() => handleSort('workName')} className="flex items-center gap-1.5 px-4 py-4 font-bold tracking-wider hover:text-slate-700 w-full group pointer-events-auto">
 Công việc {renderSortIcon('workName')}
 </button>
 </th>
 <th scope="col" className="w-[17%]">
 <button type="button" onClick={() => handleSort('assetName')} className="flex items-center gap-1.5 px-4 py-4 font-bold tracking-wider hover:text-slate-700 w-full group pointer-events-auto">
 Tài sản {renderSortIcon('assetName')}
 </button>
 </th>
 <th scope="col" className="w-[15%]">
 <button type="button" onClick={() => handleSort('status')} className="flex items-center justify-center gap-1.5 px-4 py-4 font-bold tracking-wider hover:text-slate-700 w-full group pointer-events-auto">
 Trạng thái {renderSortIcon('status')}
 </button>
 </th>
 <th scope="col" className="w-[11%]">
 <button type="button" onClick={() => handleSort('submittedAt')} className="flex items-center justify-center gap-1.5 px-4 py-4 font-bold tracking-wider hover:text-slate-700 w-full group pointer-events-auto">
 Ngày nộp {renderSortIcon('submittedAt')}
 </button>
 </th>
 <th scope="col" className="w-[18%]">
 <button type="button" onClick={() => handleSort('approvedAt')} className="flex items-center justify-center gap-1.5 px-5 py-4 font-bold tracking-wider hover:text-slate-700 w-full group pointer-events-auto">
 Phê duyệt {renderSortIcon('approvedAt')}
 </button>
 </th>
 <th scope="col" className="w-[10%] px-4 py-4 text-center font-bold tracking-wider pointer-events-none">
 Thực hiện
 </th>
 </tr>
 </thead>
 ...
 <td className="px-5 py-5"><div className="h-4 bg-slate-100 rounded w-3/4"></div></td>
 <td className="px-4 py-5"><div className="h-4 bg-slate-100 rounded w-full"></div></td>
 <td className="px-4 py-5"><div className="h-4 bg-slate-100 rounded w-1/2"></div></td>
 <td className="px-4 py-5"><div className="h-6 bg-slate-100 rounded-full mx-auto w-20"></div></td>
 <td className="px-4 py-5"><div className="h-4 bg-slate-100 rounded mx-auto w-16"></div></td>
 <td className="px-5 py-5"><div className="h-6 bg-slate-100 rounded-full mx-auto w-24"></div></td>
 <td className="px-4 py-5"><div className="h-8 bg-slate-100 rounded-lg mx-auto w-20"></div></td>
 </tr>
 ))
 ) : rows.length === 0 ? (
 <tr>
 ...
 <div className="text-slate-500 font-semibold text-base"> Không tìm thấy kết quả khớp nào </div>
 <p className="text-slate-400 text-sm">Vui lòng thử từ khóa phân phối khác hoặc thay đổi bộ lọc.</p>
 </div>
 </td>
 </tr>
 ) : (
 ...
 let dateEl = <span className="text-slate-300 ">—</span>;
 if (row.submittedAt) {
 ...
 // Actor / approver pill
 let userEl = <span className="text-slate-300 ">—</span>;
 if (row.actorId) {
 ...
 if (!isNaN(ad.getTime())) {
 approveDateEl = (
 <div className="flex items-center gap-1 mt-1 text-slate-500 opacity-90">
 ...
 <div className="font-semibold text-indigo-900 line-clamp-2">
 {row.workName}
 </div>
 </td>
 <td className="px-4 py-4">
 <div className="font-medium text-slate-500 line-clamp-2">
 {row.assetName}
 </div>
 </td>
 <td className="px-4 py-4">
 ...
 <button
 type="button"
 onClick={() => navigate(`/user/environment?assignId=${row.assignId}`)}
 className={`inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-bold transition-all duration-150 whitespace-nowrap shadow-sm border ${row.status === 'rejected' ? 'bg-rose-50 text-rose-700 border-rose-200 hover:bg-rose-100' : 'bg-indigo-50 text-indigo-700 border-indigo-200 hover:bg-indigo-100'}`}
 >
 <PlayCircle className="w-3.5 h-3.5 shrink-0" />
 {row.status === 'rejected' ? 'Sửa lại' : 'Thực hiện'}
 </button>
 ) : row.status === 'approved' ? (
 <span className="text-[10px] text-emerald-400 font-semibold">Hoàn thành</span>
 ) : (
 <span className="text-[10px] text-amber-400 font-semibold">Chờ duyệt</span>
 )}
 </td>
 </tr>
 );
 ...
 <div className="px-6 py-3 border-t border-slate-100 bg-slate-50 text-xs text-slate-400 text-right shrink-0">
