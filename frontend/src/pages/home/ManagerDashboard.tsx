...
import {
    ...
    History, BarChart3, ArrowRight, Activity, AlertCircle, ChevronRight, Zap, LayoutGrid
} from 'lucide-react';
...
const ManagerDashboard = () => {
    ...
    const fetchPendingCheckouts = useCallback(async () => {
        try {
            const res = await api.get('/attendance/pending-checkouts');
            setPendingCheckouts(res.data || []);
    ...
    const fetchDashboardData = useCallback(async () => {
        ...
        try {
            const [proj, assign, comp, users] = await Promise.all([
                api.get('/projects'),
                api.get('/allocations'),
                api.get('/manager/completed-tasks'),
                api.get('/users')
            ]);
    ...
    const handleApprove = useCallback(async (id: string) => {
        ...
        try {
            await api.post(`/attendance/approve-checkout/${id}`, { manager_id: managerId });
    ...
    const handleReject = useCallback(async (id: string) => {
        ...
        if (!managerId) return alert("Vui lòng đăng nhập lại.");
        try {
            await api.post(`/attendance/reject-checkout/${id}`, { manager_id: managerId, reason });
            fetchPendingCheckouts();
    ...
    }, [fetchDashboardData, fetchPendingCheckouts]);

    const completionRate = useMemo(() => {
        const total = stats.completedTasks + stats.activeAssignments;
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <GlassCard className="flex flex-col">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <PremiumButton variant="glass" size="sm" onClick={() => navigate('/manager/reports')} icon={<ArrowRight className="w-4 h-4" />}>Xem tất cả</PremiumButton>
                    </div>
