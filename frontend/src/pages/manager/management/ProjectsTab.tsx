...
import { Search, Plus, Trash2, MapPin, X, Copy, Calendar, ArrowUpDown, User, ChevronDown, Check, FileText } from 'lucide-react';
...
const ProjectsTab = () => {
 ...
 // Export Modal state
 const [exportModal, setExportModal] = useState<{ open: boolean; projectId: string; projectName: string }>({
   open: false,
   projectId: '',
   projectName: '',
 });
 ...
 setCloningId(null);
  ...
  const handleExportConfig = (id: string, name: string) => {
    setExportModal({ open: true, projectId: id, projectName: name });
  };
 ...
 <button
 onClick={(e) => { e.stopPropagation(); handleExportConfig(project.project_id, project.project_name); }}
 title="Xuất tóm tắt cấu hình"
 ...
 <ExportConfigModal
   ...
   projectId={exportModal.projectId}
   projectName={exportModal.projectName}
   onClose={() => setExportModal({ open: false, projectId: '', projectName: '' })}
 />
