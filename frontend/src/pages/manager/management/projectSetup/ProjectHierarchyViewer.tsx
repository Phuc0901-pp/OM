...
const ProjectHierarchyViewer: React.FC<HierarchyProps> = ({
  ...
  const [guideModal, setGuideModal] = useState<{ subWorkId: string; subWorkName: string; workName: string } | null>(null);
    ...
    <GlassCard className="p-6">
                        ...
                        return (
                          <div key={subWork.id} className="bg-white border border-slate-100 rounded-lg p-4 shadow-sm">
                            <div className="flex justify-between items-start mb-3">
                              ...
                              {/* Nút Hướng dẫn công việc */}
                              <button
                                onClick={() => setGuideModal({ subWorkId: subWork.id, subWorkName: subWork.name, workName: work.name })}
                                className="ml-3 shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-teal-50 hover:bg-teal-100 border border-teal-200 text-teal-700 text-xs font-semibold transition-all shadow-sm"
                                title="Xem / chỉnh sửa hướng dẫn công việc"
                              >
                                <BookOpen className="w-3.5 h-3.5" />
                                Hướng dẫn
                              </button>
      ...
      {guideModal && (
        <GuideLineModal
          ...
          subWorkId={guideModal.subWorkId}
          subWorkName={guideModal.subWorkName}
          workName={guideModal.workName}
        />
      )}
    </GlassCard>
  );
