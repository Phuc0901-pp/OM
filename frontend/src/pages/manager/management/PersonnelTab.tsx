...
import SmartFilterSort, { SortField, SortOrder } from '../../../components/common/SmartFilterSort';
...
const PersonnelTab = () => {
    ...
    const roleOptions = useMemo(() => {
        ...
        personnel.forEach(p => p.role && rolesSet.add(typeof p.role === 'object' ? p.role.name : p.role_name || p.role));
    ...
    const teamOptions = useMemo(() => {
        ...
        return Array.from(teamsMap.entries()).map(([id, name]) => ({ label: name, value: id }));
    }, [personnel]);
    ...
    const filteredPersonnel = useMemo(() => {
        ...
        if (selectedRoleFilter) {
            result = result.filter(user => {
                const roleName = typeof user.role === 'object' ? user.role?.name : user.role_name || user.role;
        ...
        result = [...result].sort((a, b) => {
            ...
            switch (sortField) {
                case 'name':
                    ...
                    break;
                case 'role':
                    valA = (typeof a.role === 'object' ? a.role?.name : a.role || '').toLowerCase();
                    valB = (typeof b.role === 'object' ? b.role?.name : b.role || '').toLowerCase();
                    break;
                case 'team':
        ...
        return result;
    }, [personnel, searchTerm, selectedRoleFilter, selectedTeamFilter, sortField, sortOrder]);
                ...
                {filteredPersonnel.map(user => (
                    ...
                    >
                        ...
                        <div className="p-5 pb-4 flex items-start gap-4">
                            <div className={`w-12 h-12 shrink-0 rounded-xl bg-gradient-to-br ${getAvatarGradient(user.name || user.full_name || '')} flex items-center justify-center text-lg font-bold text-white shadow-md`}>
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider ${(typeof user.role === 'object' ? user.role?.name : user.role_name || user.role) === 'engineer'
                                    ? 'bg-indigo-50 text-indigo-600 border border-indigo-100'
                                    : 'bg-slate-50 text-slate-500 border border-slate-200'
                                    }`}>
                                    {typeof user.role === 'object' ? user.role?.name : (user.role_name || user.role)}
                                </span>
                            </div>
                            {/* Menu */}
                        ...
                        <div className="px-5 py-3 border-t border-slate-100 bg-slate-50/40 flex items-center justify-between">
                            <div className="flex flex-col gap-0.5">
                                <div className="flex items-center gap-1.5 text-[11px] text-slate-500 font-medium">
                                    ...
                                    Tạo bởi: <span className="text-slate-700 font-semibold">{user.person_created?.name || user.person_created?.full_name || <span className="italic">Hệ thống</span>}</span>
                                </div>
                    ...
                    >
                        ...
                        <div className="bg-gradient-to-br from-slate-800 to-slate-900 px-6 pt-6 pb-8">
                            ...
                            <div className="flex items-center gap-4">
                                <div className={`w-14 h-14 shrink-0 rounded-2xl bg-gradient-to-br ${getAvatarGradient(contactUser.name || contactUser.full_name || '')} flex items-center justify-center text-2xl font-bold text-white shadow-lg`}>
                                    {(contactUser.name || contactUser.full_name || 'U').charAt(0).toUpperCase()}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <h2 className="text-[17px] font-bold text-white truncate">{contactUser.name || contactUser.full_name}</h2>
                                    <div className="flex items-center gap-2 mt-1">
                                        <span className="px-2 py-0.5 rounded-md bg-indigo-500/30 text-indigo-200 text-[10px] font-bold uppercase tracking-wider">
                                            {typeof contactUser.role === 'object' ? contactUser.role?.name : (contactUser.role_name || contactUser.role)}
                                        </span>
