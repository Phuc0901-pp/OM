...
import SmartFilterSort, {
 SortField,
 SortOrder,
} from "../../../components/common/SmartFilterSort";
...
interface Personnel {
 ...
 status_user?: number; // 0 = offline, 1 = online
}
...
const PersonnelTab = () => {
 ...
 const [userToEdit, setUserToEdit] = useState<Personnel | null>(null);
 const [showConfirm, setShowConfirm] = useState<{
 show: boolean;
 assignId: string;
 userId: string;
 projectName: string;
 }>({ show: false, assignId: "", userId: "", projectName: "" });
 const [showConfirmDeleteUser, setShowConfirmDeleteUser] = useState<{
 show: boolean;
 userId: string;
 userName: string;
 ...
 useEffect(() => {
 if (lastMessage && lastMessage.type === "user_status_changed") {
 const data = lastMessage.data;
 if (data && data.user_id && typeof data.status === "number") {
 setPersonnel((prev) =>
 prev.map((p) =>
 p.id === data.user_id ? { ...p, status_user: data.status } : p
 )
 );
 }
 }
 ...
 return {
 ...u,
 assigned_projects: u.assigned_projects.filter(
 (p) => p.id !== assignId,
 ),
 };
 }
 ...
 const handleDeleteUserContext = (user: Personnel) => {
 ...
 setShowConfirmDeleteUser({ show: false, userId: "", userName: "" });
 try {
 await api.delete(`/users/${userId}`);
 fetchPersonnel();
 } catch (err) {
 console.error("Failed to delete user:", err);
 alert("Đã có lỗi xảy ra khi xóa nhân sự. Vui lòng thử lại sau.");
 }
 };

 const calculateProgress = (proj: AssignedProject): number => {
 const submitted = proj.submitted_count || 0;
 const approved = proj.approved_count || 0;
 if (submitted === 0) return 0;
 return Math.round((approved / submitted) * 100);
 };

 // Lấy danh sách Roles & Teams cho filters
 ...
 p.role &&
 rolesSet.add(
 typeof p.role === "object" ? p.role.name : p.role_name || p.role,
 ),
 );
 return Array.from(rolesSet).map((r) => ({ label: r, value: r }));
 }, [personnel]);
 ...
 personnel.forEach((p) => {
 if (p.team && p.team.id) {
 teamsMap.set(p.team.id, p.team.name);
 }
 });
 return Array.from(teamsMap.entries()).map(([id, name]) => ({
 label: name,
 value: id,
 }));
 }, [personnel]);
 ...
 user.number_phone?.includes(term),
 );
 }
 ...
 result = result.filter((user) => user.team?.id === selectedTeamFilter);
 }
 ...
 case "role":
 valA = (
 typeof a.role === "object" ? a.role?.name : a.role || ""
 ).toLowerCase();
 valB = (
 typeof b.role === "object" ? b.role?.name : b.role || ""
 ).toLowerCase();
 break;
 case "team":
 ...
 break;
 case "person_created":
 valA = (a.person_created?.full_name || "").toLowerCase();
 valB = (b.person_created?.full_name || "").toLowerCase();
 break;
 case "created_at":
 ...
 const totalProjects = filteredPersonnel.reduce(
 (acc, curr) => acc + (curr.assigned_projects?.length || 0),
 0,
 );
 const activeUsers = filteredPersonnel.filter(
 (p) => p.assigned_projects?.length > 0,
 ).length;
 ...
 <div
 className={`w-12 h-12 rounded-2xl flex items-center justify-center bg-gradient-to-br ${s.from} ${s.to} text-white shadow-md`}
 >
 ...
 {/* 3. Personnel Grid */}
 <div
 className={`grid gap-5 ${viewMode === "grid" ? "grid-cols-1 md:grid-cols-2 lg:grid-cols-3" : "grid-cols-1"}`}
 >
 ...
 <div
 className={`w-12 h-12 shrink-0 rounded-xl bg-gradient-to-br ${getAvatarGradient(user.name || user.full_name || "")} flex items-center justify-center text-lg font-bold text-white shadow-md`}
 >
 ...
 (typeof user.role === "object"
 ? user.role?.name
 : user.role_name || user.role) === "engineer"
 ...
 {/* Status Pill */}
 <div className="shrink-0 flex items-center pr-1 mt-0.5">
 <span
 className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wider uppercase border shadow-sm ${
 user.status_user === 1
 ...
 user.status_user === 1 ? "bg-emerald-500 animate-pulse" : "bg-slate-300"
 }`}
 />
 {user.status_user === 1 ? "Online" : "Offline"}
 </span>
 </div>
 {/* Menu */}
 <div
 className="shrink-0 relative"
 ref={activeMenuId === user.id ? menuRef : undefined}
 >
 <button
 onClick={(e) => {
 e.stopPropagation();
 ...
 className="w-full text-left px-4 py-2.5 text-sm text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 flex items-center gap-2 transition-colors"
 >
 ...
 <button
 onClick={() => handleDeleteUserContext(user)}
 ...
 >
 <Trash2 className="w-3.5 h-3.5" /> Xóa nhân sự
 </button>
 </div>
 )}
 </div>
 ...
 <span
 className="text-[13px] font-medium text-slate-700 truncate max-w-[75%]"
 title={proj.project?.project_name}
 >
 ...
 {user.person_created?.name ||
 user.person_created?.full_name || (
 ...
 <div
 className={`w-14 h-14 shrink-0 rounded-2xl bg-gradient-to-br ${getAvatarGradient(contactUser.name || contactUser.full_name || "")} flex items-center justify-center text-2xl font-bold text-white shadow-lg`}
 >
 {(contactUser.name || contactUser.full_name || "U")
 .charAt(0)
 .toUpperCase()}
 </div>
 <div className="flex-1 min-w-0">
 <h2 className="text-[17px] font-bold text-white truncate">
 {contactUser.name || contactUser.full_name}
 </h2>
 ...
 {typeof contactUser.role === "object"
 ? contactUser.role?.name
 : contactUser.role_name || contactUser.role}
 </span>
 ...
 onClick={() =>
 window.open(
 `https://mail.google.com/mail/?view=cm&fs=1&to=${contactUser.email}`,
 "_blank",
 )
 }
 ...
 onClick={() =>
 window.open(
 `https://zalo.me/${contactUser.number_phone || "0909123456"}`,
 "_blank",
 )
 }
 ...
 {contactUser.number_phone || "Chưa cập nhật"}
 </p>
 </div>
 <span className="shrink-0 px-2 py-1 rounded-md bg-emerald-100 text-emerald-700 text-[10px] font-bold">
 ZL
 </span>
 </button>
 </div>
 ...
 {showConfirmDeleteUser.show && (
 <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
 <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 border border-slate-100 animate-fade-in">
 ...
 "{showConfirmDeleteUser.userName}"
 </strong>{" "}
 không? Hành động này sẽ ảnh hưởng đến các phân công hiện tại của
 họ.
 </p>
 ...
 className="flex-1 px-4 py-2.5 bg-slate-100 rounded-xl text-slate-600 text-sm font-semibold hover:bg-slate-200 transition-colors"
 >
 Hủy bỏ
 </button>
 <button
 onClick={confirmDeleteUser}
 className="flex-1 px-4 py-2.5 bg-red-500 text-white rounded-xl text-sm font-semibold hover:bg-red-600 transition-colors shadow-md shadow-red-500/20"
 >
 Xác nhận xóa
 </button>
 </div>
 ...
 Bạn có chắc muốn hủy phân công dự án{" "}
 <strong className="text-slate-700">
