...
const HistoryPage = () => {
    ...
    const [activeTab, setActiveTab] = useState<'deleted_assignments' | 'work_schedule'>(
        tabParam === 'work_schedule' ? 'work_schedule' : 'deleted_assignments'
    );
    ...
    const scheduleColumns: ColumnDef<AttendanceRecord>[] = useMemo(() => [
        ...
        },
        {
            ...
            cell: (val: any, row: any) => (
                <div>
                    ...
                    {row.project?.location && <div className="text-xs text-slate-500 ml-4">{row.project?.location}</div>}
            ...
            cell: (val: any, row: any) => (
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        {row.site_status === 1 && <div className="text-[10px] font-bold text-green-600">Site Status: ON</div>}
                    </div>
                </div>
            )
        },
        {
            ...
            cell: (val: any, row: any) => {
                const isRejected = !!row.checkout_rejected_at; // Assuming fields map correctly
                        ...
                        <div>
                            <div className={`flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-md mb-1 w-fit ${isRejected ? 'bg-red-50 text-red-700' :
                                isApproved ? 'bg-indigo-50 text-indigo-700' :
                                    row.checkout_requested ? 'bg-amber-50 text-amber-700' : 'text-slate-400'
                                }`}>
                                <Clock className="w-3 h-3" />
                                {row.date_checkout ? new Date(row.date_checkout).toLocaleTimeString('vi-VN') :
                                    isRejected ? 'Bị từ chối' :
                                        row.checkout_requested ? 'Chờ duyệt' : '---'}
                            </div>
                        </div>
                    </div>
                );
    ...
    const filteredSchedule = useMemo(() => {
        ...
        if (attendanceIdParam) {
            return data.filter((item: AttendanceRecord) => item.id === attendanceIdParam);
        }
    ...
    const totalPages = Math.ceil((activeTab === 'deleted_assignments' ? filteredHistory.length : filteredSchedule.length) / pageSize) || 1;

    // Reset page if out of bounds (e.g. after delete/filter)
    useEffect(() => {
        if (page > totalPages) {
            setPage(1);
        }
        ...
        <div className="p-4 md:p-8 space-y-6 pb-24">
            <GlassCard className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-3 bg-slate-100/50 p-1.5 rounded-xl border border-white/50">
                    <button onClick={() => { setActiveTab('deleted_assignments'); setPage(1); }} className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'deleted_assignments' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                        Lịch sử Xóa
                    </button>
                    <button onClick={() => { setActiveTab('work_schedule'); setPage(1); }} className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'work_schedule' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                        Lịch trình
                    </button>
                </div>
            </GlassCard>
                    ...
                    <AnimatePresence mode="wait">
                        {activeTab === 'deleted_assignments' ? (
                            <motion.div key="history" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                <PremiumTable columns={historyColumns} data={paginatedHistory} isLoading={loadingHistory} keyField="id" />
                            ...
                            <motion.div key="schedule" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                <PremiumTable columns={scheduleColumns} data={paginatedSchedule} isLoading={loadingSchedule} keyField="id" />
