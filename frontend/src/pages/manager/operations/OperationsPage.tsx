...
const ManagerOperationsPage = () => {
                    ...
                    detailAssigns.forEach((t: any) => {
                        ...
                        const getLastDate = (dateField: any) => {
                            ...
                            if (Array.isArray(dateField)) {
                                ...
                                try { arr = JSON.parse(dateField); } catch (e) { }
                            }
    ...
    const handleBulkUpdateStatus = async (ids: string[], status: number, note?: string) => {
        try {
            ...
            if (status === 1) {
                await Promise.all(ids.map(id => api.post(`/details/${id}/approve`, { note_approval: note || "Duyệt" })));
            } else if (status === -1) {
                await Promise.all(ids.map(id => api.post(`/details/${id}/reject`, { note_reject: note || "Từ chối" })));
            }
            ...
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                <div className="relative overflow-hidden bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl p-6 md:p-8 shadow-xl dark:shadow-2xl border border-white/20 dark:border-white/5 transition-colors">
                    ...
                    <div className="relative z-10 flex flex-col gap-1">
                        ...
                        <p className="text-gray-600 dark:text-slate-400 font-medium">Bảng theo dõi và vận hành công việc</p>
                    </div>
                </div>
