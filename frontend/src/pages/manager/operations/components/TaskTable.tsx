...
import { Search, ChevronRight, LayoutGrid, CheckSquare, Square, Check, X, Loader2, CheckCircle2, XCircle, Filter, Calendar, Folder, User as UserIcon, ChevronDown, ChevronUp, Image as ImageIcon, CheckCircle } from 'lucide-react';
...
const getStatusBadge = (statusString?: string) => {
    switch (statusString || "0000") {
        ...
        case '1001': return <span className="px-2 py-1 rounded-full text-[10px] font-bold bg-cyan-50 text-cyan-600 border border-cyan-200">Đã nộp lại</span>;
...
const TaskTable: React.FC<TaskTableProps> = ({
    ...
    const approvableTasks = useMemo(() => tasks.filter(t => t.statusString === '1100' || t.statusString === '1001'), [tasks]);
            ...
            if (checkedIds.has(t.id)) {
                if (t.subTasks && t.subTasks.length > 0) {
                    t.subTasks.forEach(st => {
                        if (st.statusString === '1100' || st.statusString === '1001') allIds.push(st.id);
                    });
                } else {
                    if (t.statusString === '1100' || t.statusString === '1001') allIds.push(t.id);
                }
            }
        });
        return allIds;
                    ...
                    >
                        ...
                        <div className="p-4 md:p-5 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white/50 backdrop-blur-md sticky top-0 z-20">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h2 className="text-xl font-extrabold text-slate-800 tracking-tight">{tasks.length > 0 ? tasks[0].projectName : 'Chi tiết phân bổ'}</h2>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    <AnimatePresence>
                                        {tasks.map((task, index) => {
                                            const displayStationName = task.assetName || '-';
                                                ...
                                                >
                                                    ...
                                                    <td className="px-3 py-3 md:px-6 md:py-4 min-w-[200px]">
                                                        <div className="flex flex-col gap-1">
                                                            <div className="text-sm font-medium text-slate-600">{task.subWorkName || '-'}</div>
                                                            {task.processName && <span className="text-xs text-slate-400">{task.processName}</span>}
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td className="px-3 py-3 md:px-6 md:py-4 hidden md:table-cell text-xs whitespace-nowrap">
                                                        {task.submittedAt && !isNaN(parseSafeDate(task.submittedAt).getTime()) ? (
                                                            <span className="font-medium text-slate-600 bg-slate-100 px-2 py-1 rounded-md">{format(parseSafeDate(task.submittedAt), 'dd/MM/yyyy HH:mm')}</span>
                                                    ...
                                                    <td className="px-3 py-3 md:px-6 md:py-4 hidden md:table-cell text-xs whitespace-nowrap">
                                                        {task.approvalAt && !isNaN(parseSafeDate(task.approvalAt).getTime()) ? (
                                                            <span className="text-emerald-600 font-medium">{format(parseSafeDate(task.approvalAt), 'dd/MM/yyyy HH:mm')}</span>
                                                        ) : task.rejectedAt && !isNaN(parseSafeDate(task.rejectedAt).getTime()) ? (
                                                            <span className="text-rose-600 font-medium">{format(parseSafeDate(task.rejectedAt), 'dd/MM/yyyy HH:mm')}</span>
                                                        ) : <span className="text-slate-300 italic">-</span>}
                                                    </td>
                                                                ...
                                                                <button
                                                                    onClick={async (e) => {
                                                                        ...
                                                                        if (idsToApprove.length > 0) {
                                                                            await onBulkUpdateStatus(idsToApprove, 1, 'Quick Approved');
                                                                            fetchTasks();
                                                                        }
                                                                    }}
                                                                    className="p-1.5 text-emerald-500 hover:text-white hover:bg-emerald-500 rounded-lg transition-all shadow-sm border border-emerald-100"
                                                                    title="Duyệt nhanh"
                                                                >
