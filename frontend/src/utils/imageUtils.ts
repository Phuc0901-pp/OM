export const getImageUrl = (item: string | Blob | undefined | null): string => {
 ...
 return item.type.startsWith('video/') ? url + '#video' : url;
 }
 ...
 // Detect MinIO Object Keys stored without 'minio' prefix (e.g. '2024/01/...' or 'project-name/2026/03/...')
 if (/^\d{4}\//.test(path) || /\/\d{4}\//.test(path)) {
