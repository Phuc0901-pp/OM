...
func (s *UserService) CreateUser(email, password, fullName, roleIDStr, teamIDStr, phoneNumber, telegramChatID string) (*domain.User, error) {
	...
	// Re-fetch to get preloads (Role, Team)
	// Or we can simple pre-set them if we had objects, but we only have IDs.
	// For API response completeness, let's just return what we have or refetch. 
	// Simple return is fine for now, standard for create.
