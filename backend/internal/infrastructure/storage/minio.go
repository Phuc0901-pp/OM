...
func NewMinioClient() (*MinioClient, error) {
	...
	// Add retry loop for checking bucket existence, since MinIO might not be up yet
	var exists bool
	var bucketErr error
	maxRetries := 15
	for i := 0; i < maxRetries; i++ {
		...
		fmt.Printf("[MinIO] Waiting for MinIO to be ready... (Attempt %d/%d): %v\n", i+1, maxRetries, bucketErr)
		time.Sleep(3 * time.Second)
	}

	if bucketErr != nil {
		return nil, fmt.Errorf("failed to check bucket existence after %d retries: %w", maxRetries, bucketErr)
	}
...
func (m *MinioClient) UploadBytes(data []byte, objectName string, contentType string) (string, error) {
	...
	// Return the FULL URL to maintain backward compatibility with the mobile app
	// which expects "http://minio:2603/dev/..." format.
	endpointURL := m.Client.EndpointURL()
	fullURL := fmt.Sprintf("%s/%s/%s", endpointURL.String(), m.Bucket, objectName)
	return fullURL, nil
}

// UploadStream streams data directly from an io.Reader to MinIO without buffering
// the entire file into RAM first. Use this for large files (images/videos) to avoid
// memory exhaustion on the server when multiple concurrent uploads happen.
// fileSize must be provided accurately; pass -1 only if truly unknown (slower).
func (m *MinioClient) UploadStream(r io.Reader, fileSize int64, objectName string, contentType string) (string, error) {
	if m == nil || m.Client == nil {
		return "", fmt.Errorf("minio client is not initialized")
	}
	...
	_, err := m.Client.PutObject(
		...
		minio.PutObjectOptions{
			...
			// Disable checksum CRC that requires the full body upfront
...
func (m *MinioClient) DeleteFolder(prefix string) (int, error) {
	if m == nil || m.Client == nil {
		return 0, fmt.Errorf("minio client is not initialized")
	}
	...
	// Feed objects into removal channel in a goroutine
	go func() {
		defer close(objectsCh)
		for object := range m.Client.ListObjects(ctx, m.Bucket, listOpts) {
			if object.Err != nil {
				continue
			}
			objectsCh <- object
	...
	removeOpts := minio.RemoveObjectsOptions{GovernanceBypass: true}
	errCh := m.Client.RemoveObjects(ctx, m.Bucket, objectsCh, removeOpts)

	count := 0
	for removeErr := range errCh {
		if removeErr.Err != nil {
			return count, fmt.Errorf("failed to delete object %s: %w", removeErr.ObjectName, removeErr.Err)
		}
		count++
	}
	return count, nil
