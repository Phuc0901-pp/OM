...
type AttendanceHandler struct {
	...
	statsHandler *StatsHandler // For cache invalidation
}

func NewAttendanceHandler(service *services.AttendanceService, statsHandler *StatsHandler) *AttendanceHandler {
	return &AttendanceHandler{service: service, statsHandler: statsHandler}
}
...
func (h *AttendanceHandler) CheckInWithPhotos(c *gin.Context) {
	var req struct {
		...
		AssignID         *string     `json:"assign_id"`  // Optional: link to specific assignment
		PersonnelPhoto   interface{} `json:"personnel_photo"`
		IDCardFront      interface{} `json:"id_card_front"`
		IDCardBack       interface{} `json:"id_card_back"`
		SafetyCardFront  interface{} `json:"safety_card_front"`
		SafetyCardBack   interface{} `json:"safety_card_back"`
	...
	if req.AssignID != nil && *req.AssignID != "" {
		id, err := uuid.Parse(*req.AssignID)
		if err == nil {
			assignID = &id
		}
	}

	attendance, err := h.service.CheckInWithPhotos(userID, projectID, assignID, photos, req.Address)
	if err != nil {
...
func (h *AttendanceHandler) CheckIn(c *gin.Context) {
	...
	attendance, err := h.service.CheckIn(userID, projectID, assignID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// Optional query param: ?assign_id=<uuid> to get attendance for a specific assignment
func (h *AttendanceHandler) GetTodayAttendance(c *gin.Context) {
	...
	var assignID *uuid.UUID
	if assignIDStr := c.Query("assign_id"); assignIDStr != "" {
		id, err := uuid.Parse(assignIDStr)
		if err == nil {
			assignID = &id
		}
	}
	...
	if assignID != nil {
		attendance, err = h.service.GetTodayAttendanceByAssign(userID, assignID)
	} else {
		attendance, err = h.service.GetTodayAttendance(userID)
	}
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"data": nil, "message": "No attendance record found for today"})
		return
	}
...
func (h *AttendanceHandler) ApproveCheckout(c *gin.Context) {
	...
	// Invalidate manager dashboard cache so stats refresh immediately
	if h.statsHandler != nil {
		h.statsHandler.InvalidateManagerCache(req.ManagerID)
		h.statsHandler.InvalidateAdminCache()
	}
...
// Accepts optional ?manager_id= to filter by manager's staff only
func (h *AttendanceHandler) GetPendingCheckouts(c *gin.Context) {
	...
	attendances, err := h.service.GetPendingCheckoutRequests(managerID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// GetByAssignDates handles GET /api/attendance/by-assign-dates?assign_id=...&dates[]=YYYY-MM-DD&dates[]=...
func (h *AttendanceHandler) GetByAssignDates(c *gin.Context) {
	assignIDStr := c.Query("assign_id")
	if assignIDStr == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "assign_id is required"})
		return
	}
	assignID, err := uuid.Parse(assignIDStr)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid assign_id"})
		return
	}

	dates := c.QueryArray("dates[]")
	if len(dates) == 0 {
		// Try without brackets (some clients omit them)
		dates = c.QueryArray("dates")
	}

	attendances, err := h.service.GetByAssignAndDates(assignID, dates)
	if err != nil {
