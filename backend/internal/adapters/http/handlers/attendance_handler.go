...
func (h *AttendanceHandler) CheckInWithPhotos(c *gin.Context) {
	var req struct {
		...
		PersonnelPhoto   string   `json:"personnel_photo"`
	...
	attendance, err := h.service.CheckInWithPhotos(userID, projectID, photos)
	if err != nil {
...
func (h *AttendanceHandler) RequestCheckout(c *gin.Context) {
	...
	attendance, err := h.service.RequestCheckout(userID)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
