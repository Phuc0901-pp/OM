...
func (h *StationHandler) CreateStation(c *gin.Context) {
	var req struct {
		...
		ProjectID      string `json:"project_id"`      // Frontend key
...
func (h *StationHandler) UploadGuideFile(c *gin.Context) {
    ...
    fmt.Printf("DEBUG: Station ID: %s\n", station.ID)
    fmt.Printf("DEBUG: Station ProjectID: %s\n", station.ProjectID)
    fmt.Printf("DEBUG: Station MainCategoryID: %s\n", station.MainCategoryID)
    if station.Project == nil {
        fmt.Println("DEBUG: Station.Project is NIL")
    } else {
        fmt.Printf("DEBUG: Station.Project.Name: %s\n", station.Project.ProjectName)
    }
    if station.MainCategory == nil {
        fmt.Println("DEBUG: Station.MainCategory is NIL")
    }
	...
	// Get path info
	if station.Project == nil {
		fmt.Printf("Error: Project info missing for station %s\n", station.ID)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Project info missing for this station"})
		return
	}
	if station.MainCategory == nil {
		fmt.Printf("Error: Main Category info missing for station %s\n", station.ID)
	...
	for _, file := range files {
        ...
        fmt.Printf("Uploading file to MinIO: %s\n", objectName)
		...
		if h.MinioClient == nil {
			fmt.Printf("Error: MinIO Client is not initialized (check server logs)\n")
