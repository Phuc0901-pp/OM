...
// Restore - xóa cột deleted_at để khôi phục một User
func (r *userRepository) Restore(id uuid.UUID) error {
	return r.db.Unscoped().Model(&domain.User{}).Where("id = ?", id).Updates(map[string]interface{}{
		"deleted_at": nil,
	}).Error
}

// PermanentDelete - xóa vĩnh viễn User khỏi DB (Hard Delete)
func (r *userRepository) PermanentDelete(id uuid.UUID) error {
	return r.db.Unscoped().Where("id = ?", id).Delete(&domain.User{}).Error
}

// BulkRestore - khôi phục nhiều User cùng lúc
func (r *userRepository) BulkRestore(ids []uuid.UUID) error {
	if len(ids) == 0 {
		return nil
	}
	return r.db.Unscoped().Model(&domain.User{}).Where("id IN ?", ids).Updates(map[string]interface{}{
		"deleted_at": nil,
	}).Error
}
