...
func (r *AttendanceRepository) CheckIn(userID uuid.UUID, projectID *uuid.UUID) (*domain.Attendance, error) {
	now := time.Now()
	...
	// However, to keep it simple and consistent with "One 'Shift' per day" usually, or allowing multiple?
	// The user request implies "Mỗi lần user checkin... lưu thông tin". This implies multiple rows are possible if they check in/out multiple times.
	...
	if err == gorm.ErrRecordNotFound {
		...
		// For this specific system, let's assume if it's already checked out, we create a new session.
		...
		// Otherwise, update existing record (e.g. they are just updating info or re-checking in without checkout?)
		// Or maybe they are switching projects?
		// If they are switching projects without checkout, we should probably update project ID?
		// Let's just update the status and project.
